...
func AlyUnApportionBill(c *gin.Context) {
	//var date string
	...
	db := orm.BillDb
	if err := db.Model(&mode.AlyBill{}).Where("apportion_depart IS NULL").Count(&total).Error; err != nil{
	...
	if startTime != "" {
		...
		if err := db.Model(&mode.AlyBill{}).Where("apportion_depart IS NULL").Order("id desc").Limit(pageSize).Offset(offset).Find(&data).Error; err != nil {
			c.JSON(http.StatusBadRequest, gin.H{
				"code":    400,
				"message": "抱歉未找到相关信息",
			})
		...
		if err := db.Table("(?) as o", db.Model(&mode.AlyBill{}).Select("payment_amount").Where("apportion_depart IS NULL").Order("id desc").Limit(pageSize).Offset(offset)).Select("sum(payment_amount) as payCount").Find(&payCount).Error; err != nil {
			c.JSON(http.StatusOK, gin.H{
				"code":    500,
				"message": "查询数据异常",
			})
